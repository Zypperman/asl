        ...
        if not (cap.isOpened()):
            ...
            if len(boxes) > 0:
                ...
                if (
                    ...
                    and label == words[current_word][current_letter]
                ):
                    current_letter += 1
                ...
                frame = cv2.rectangle(
                    frame, (int(x), int(y)), (int(w), int(h)), (36, 255, 12), 1
                )
                cv2.putText(
                    frame,
                    f"Label: {label}",
            ...
            if current_word != len(words):
                ...
                cv2.putText(
                    frame,
                    f"Current Word: {words[current_word]}",
                    (10, 50),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.9,
                    (36, 255, 12),
                ...
                cv2.putText(
                    frame,
                    f"Current Letter: {words[current_word][current_letter]}",
                    (10, 80),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.9,
                    (36, 255, 12),
